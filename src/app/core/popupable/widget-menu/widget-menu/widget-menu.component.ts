...
export class WidgetMenuComponent implements AfterViewInit, OnInit {
  ...
  ngAfterViewInit() {
    ...
    this.isOpen$ = this.componentSubj.asObservable().pipe(map(cmp => !!cmp));

    this.closedPopup.asObservable().subscribe(() => {
